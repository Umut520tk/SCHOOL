...
void GetInTheBus(BusStop *stop,Bus *bus){
    ...
    if (bus->passengerCount < MAX_PASSENGER)
    {
        for (size_t i = 0; i < stop->passengerCount; i++)
        {
            ...
            if (pass != NULL && pass->LuaggageCount + bus->luggagesTop + 1 < MAX_LUGGAGE)
            {
...
void UnloadLuggages(Bus *bus){
    ...
    if (bus->isUnloadingDone)
    {
        ...
        if (bus->waitingLuggages[bus->waitinLuggagesIndex].id != -1)
        {
            bus->luggagesTop++;
            bus->luggages[bus->luggagesTop].id = bus->waitingLuggages[bus->waitinLuggagesIndex].id;
            bus->luggages[bus->luggagesTop].owner = bus->waitingLuggages[bus->waitinLuggagesIndex].owner;

            bus->waitingLuggages[bus->waitinLuggagesIndex].id = -1;
            bus->waitingLuggages[bus->waitinLuggagesIndex].owner = NULL;
        ...
        if (bus->luggagesTop > -1 && bus->luggageToBeUnloadedCount > 0)
        {
            for (size_t i = 0; i < MAX_LUGGAGE; i++)
            {
        ...
        else if (bus->luggageToBeUnloadedCount > 0){
            bus->luggageToBeUnloadedCount = 0;
            bus->luggagesTop = -1;
            bus->isUnloadingDone = false;
            bus->luaggesUnloadOperations = false;
    ...
    while(1) {
        if(selectedBusForDisplay != NULL) {
            ...
            for (size_t i = 0; i < selectedBusForDisplay->passengerCount; i++)
            {
                ...
                {
                    printf("\033[%d;88H%3d",i+16,temp->luggages[0].id);
                    if (temp->luggages[1].owner != NULL)
                    {
                        printf("\033[%d;91H,%3d",i+16,temp->luggages[1].id);
                        printf("\033[%d;95H)",i+16);
                    }
                    else{
                        printf("\033[%d;91H)",i+16);
                    }
                }
                else{
                    printf("\033[%d;88H-",i+16);
                    printf("\033[%d;89H)",i+16);
                }
                temp = temp->next;
            } 

            printf("\033[24;82H\033[K");
            printf("\033[24;82HBus [%s] Luggages", selectedBusForDisplay->Name);

            for (size_t i = 0; i < 8; i++)
            {
                printf("\033[%d;100H\033[K",24 - i);
                printf("\033[%d;100H|",24 - i);
                printf("\033[%d;104H\033[K",24 - i);
                printf("\033[%d;104H|",24-i);
            }
            printf("\033[25;100H-----");
            ...
            for (size_t i = 0; i < selectedBusForDisplay->luggagesTop+1; i++)
            {
                ...
                {
                    printf("\033[%d;101H%d",24 - i,selectedBusForDisplay->luggages[i].id);
                }
            }
        }
        Sleep(1000);
